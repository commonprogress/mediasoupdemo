...
public class BindingAdapters {
    ...
    public static void inviteLink(TextView view, String inviteLink) {
        view.setVisibility(Utils.isEmptyString(inviteLink) ? View.INVISIBLE : View.VISIBLE);
    }
    ...
    public static void deviceInfo(TextView view, DeviceInfo deviceInfo) {
        ...
        if (!Utils.isEmptyString(deviceInfo.getFlag())) {
    ...
    public static boolean render(SurfaceViewRenderer renderer, VideoTrack track, RoomConstant.PeerState peerState, boolean isConnected) {
        LogUtils.d(TAG, "P2PConnect edias_render: " + (track != null && isConnected ? "VISIBLE" : "GONE") + ", isConnected:" + isConnected + ", peerState:" + peerState + ", null == renderer:" + (null == renderer));
        if (null != renderer) {
            if (track != null && track.enabled() && isConnected) {
                if (peerState != RoomConstant.PeerState.CLOSED) {
    ...
    public static void renderEmpty(View renderer, VideoTrack track, RoomConstant.PeerState peerState, boolean isConnected) {
        LogUtils.d(TAG, "edias_render_empty: " + (track != null && isConnected ? "GONE" : "VISIBLE") + ", isConnected:" + isConnected + ", peerState:" + peerState);
        if (track == null || !track.enabled() || !isConnected) {
            if (peerState != RoomConstant.PeerState.CLOSED) {
                renderer.setVisibility(View.VISIBLE);
            }
