...
public class Peer implements AbsWebSocketTransport.Listener {
    ...
    public void request(String method, String roomId, String peerId, String toId, String data, ClientRequestHandler clientRequestHandler) {
        try {
            request(method, roomId, peerId, toId, new JSONObject(data), clientRequestHandler);
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
     ...
     */
    public void request(
            String method, String roomId, String peerId, String toId, @NonNull JSONObject data, ClientRequestHandler clientRequestHandler) {
        JSONObject request = Message.createRequest(method, roomId, peerId, toId, data);
        long requestId = request.optLong("id");
        Logger.d(TAG, String.format("request() [method:%s, data:%s]", method, data.toString()));
        String payload = mTransport.sendMessage(request);
    ...
    public void notify(String method, String roomId, String peerId, String toId, String data) {
        try {
            notify(method, roomId, peerId, toId, new JSONObject(data));
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
     ...
     * @param data
     */
    public void notify(String method, String roomId, String peerId, String toId, JSONObject data) {
        JSONObject notification = Message.createNotification(method, roomId, peerId, toId, data);
        Logger.d(TAG, String.format("notify() [method:%s]", method));
        mTransport.sendMessage(notification);
    }
    ...
    private void handleRequest(final Message.Request request) {
        if (null == mListener) {
            Logger.e(
                    TAG, "handleRequest mListener:" + mListener + ", mTransport:" + mTransport + ", request:" + request);
            return;
        }
        mListener.onRequest(
                request,
    ...
    private void handleResponse(Message.Response response) {
        if (null == response) {
            Logger.e(
                    TAG, "handleResponse sent request null == response");
            return;
        }
        ...
        //响应消息 解析
        if (response.isOK()) {
            sent.resolve(null == response.getData() ? null : response.getData().toString());
        } else {
    ...
    private void handleNotification(Message.Notification notification) {
        if (null == mListener) {
            Logger.e(
                    TAG, "handleNotification null == mListener");
            return;
        }
